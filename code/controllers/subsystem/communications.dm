...
/datum/controller/subsystem/communications/proc/make_announcement(mob/living/user, is_silicon, input, syndicate, list/players)
	...
	if(is_silicon)
		...
		if(syndicate)
			priority_announce(html_decode(message_data["message"]), null, 'sound/misc/announce_syndi.ogg', ANNOUNCEMENT_TYPE_SYNDICATE, has_important_message = TRUE, players = players, color_override = "red") // MONKESTATION EDIT CHANGE OLD -- We dont have TTS (BUT I DOOOOO)
//			priority_announce(html_decode(message_data), null, 'sound/misc/announce_syndi.ogg', ANNOUNCEMENT_TYPE_SYNDICATE, has_important_message = TRUE, players = players, color_override = "red") // MONKESTATION EDIT CHANGE NEW
			...
			priority_announce(html_decode(message_data["message"]), null, 'sound/misc/announce.ogg', ANNOUNCEMENT_TYPE_CAPTAIN, has_important_message = TRUE, players = players) // MONKESTATION EDIT CHANGE OLD -- We dont have TTS (BUT I DOOOOO)
